...
<div class="row">
    <div class="col-md-11">
        <form action="{{  route('testimoniHiddenGem.update', ['testimoniHiddenGem'=>$testimoni->id]) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="card mb-4">
                <h5 class="card-header">Edit Testimoni User</h5>
                <div class="card-body">
                    <div class="mb-3">
                        <label for="input_post_title" class="form-label">Nama</label>
                        <input id="input_post_title" name="name" type="text" placeholder="" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name', $testimoni->name) }}" />
                        @error('name')
