...
class HomeController extends Controller
{
    ...
    public function detail(Request $request, $id, $trip)
    {
         ...
         * ambil detail trip
         */
        $detailTrip = Trip_categories::with(['place_trip_categories:id,trip_categories_id,place_categories_id', 'place_trip_categories.place_categories:id,title,slug', 'place_trip_categories_cities:id,trip_categories_id,place_categories_id', 'place_trip_categories_cities.place_categories:id,title,images',
         'place_trip_categories_cities.pick_hidden_gem:id,place_categories_id,place_categories_categories_cities_id,hidden_gem_id', 'place_trip_categories_cities.pick_hidden_gem.hidden_gems:id,slug,image_desktop,image_mobile,title', 'hashtag_place_trip', 'trip_include:title,icon_image,trip_cat_id', 'trip_exclude:title,icon_image,trip_cat_id', 'testimoniUser'])->where('slug', '=', $trip)->where('status', 'publish')->first(['id', 'title', 'slug', 'thumbnail', 'description', 'itinerary', 'price', 'day', 'night', 'seat', 'link_g_drive', 'date_from', 'date_to', 'banner', 'trip_review', 'trip_star']);
    ...
    public function addToCart(Request $request)
    {
        ...
        $uniqueToken =Uuid::uuid4()->toString();
        ...
        if (!Auth::user()) {
            ...
            return redirect()->route('booking',$uniqueToken);
        } else {
    ...
    public function hiddemGem($id, $slug)
    {
        ...
        $tripReview = review_hidden_gem::where('hidden_gem_id', '=', $hiddenGem->id)->get();

        $tripReviewSatu = 'lorem ipsum bla bala bla...';
        $count = 120;
        if ($tripReview->count() != 0) {
            $tripReviewSatu = $tripReview[0]->description;
            $count = $tripReview->count();
        }
    ...
    public function searchTripByDates(Request $request)
    {
        ...
        if ($reservationsq->count() != 0) {
            foreach ($reservationsq as $reservation) {
                $reservation['date_from_result'] = date('d M Y', strtotime($reservation->date_from));
    ...
    public function profileUpdate(Request $request)
    {
        ...
        try {
            ...
            DB::rollBack();
            Alert::error('Edit Profile', 'error' . $th->getMessage());
            return redirect()->back()->withInput($request->all());
        } finally {
            DB::commit();
        }
    ...
    public function booking1($token)
    {

        //ambil cart berdasarkan uuid
        $newCart = Cart::with(['trip:id,title,seat,thumbnail,date_from,date_to,price,dp_price,installment1,installment2,installment3,visa,tipping,total_tipping,day'])->where('unique_token', '=', $token)->first();
        
        $uniqueToken = $token;
        $status = $newCart->status_order;
        ...
        if ($dayRange >= 1 and $dayRange <= 44) {
            ...
            $monthly = [
                [
                    ...
                    $hari = $dayRange,
                    $priceDP =($newCart->trip->dp_price + $newCart->trip->installment1) * $newCart->qty,
                    $qty = $newCart->qty
                ]
            ];
        } elseif ($dayRange >= 45 and $dayRange <= 60) {
            $months = 2;

            $monthly = [
                [
                    // $price = ($newCart->trip->dp_price + $newCart->trip->installment1) * $newCart->qty,
                    $price = $newCart->trip->dp_price + $newCart->trip->installment1,
                    $perMonth = 'DP / Uang Muka',
                    $visaperMonth = 0,
                    $tippingPerMonth = 0,
                    $totalPerMonth = ($price * $newCart->qty) + $visaperMonth + $tippingPerMonth,
                    $hari = $dayRange,
                    $priceDP =($newCart->trip->dp_price + $newCart->trip->installment1) * $newCart->qty,
                    $qty = $newCart->qty
                ],
        ...
        return view('web.booking.index3', compact('newCart', 'months', 'pricePerMonths', 'monthly', 'uniqueToken', 'status'));
    }
    ...
    {   
        if (!Auth::user()) {
            $order = Cart::where('unique_token', '=', $request->uniqueToken);
            $order->update([
                'qty' => $request->qty,
        ...
        $newCart = Cart::with(['trip:id,title,seat,thumbnail,date_from,date_to,price,installment1,installment2,installment3,visa,total_tipping,tipping,dp_price,link_g_drive'])->where('unique_token', '=', $request->uniqueToken)->first();
        ...
        if ($dayRange >= 31 and $dayRange <= 60) {
            ...
            $monthly = [
                ...
                [
                    ...
                    $due_date_satu = NULL,
                    $due_date_dua  = NULL,
                    $email_reminder_date = date('Y-m-d', strtotime($newCart->trip->date_from . ' -' . 1 * 37 . 'days')) 
            ...
            $monthly = [
                [
                    ...
                    $visaperMonth = 0,
                    $tippingPerMonth = 0,
                    $due_date_satu = date('Y-m-d', strtotime($newCart->trip->date_from . ' -' . 1 * 50 . 'days')),
                    $due_date_dua  = date('Y-m-d', strtotime($newCart->trip->date_from . ' -' . 1 * 30 . 'days')),
                    $email_reminder_date = NULL
                ],
                [
                    $price = $newCart->trip->installment1,
                    $perMonth = date('Y-m-d', strtotime($newCart->trip->date_from . ' -' . 1 * 50 . 'days')),
                    $visaperMonth = $newCart->trip->visa,
                    $tippingPerMonth = 0,
                    $due_date_satu = NULL,
                    $due_date_dua  = NULL,
                    $email_reminder_date = date('Y-m-d', strtotime($newCart->trip->date_from . ' -' . 1 * 55 . 'days'))
                ],
                [
                    $price = $newCart->trip->installment2,
                    $perMonth = date('Y-m-d', strtotime($newCart->trip->date_from . ' -' . 1 * 30 . 'days')),
            ...
            for ($i = 1; $i <= $request->months; $i++) {
                ...
                try {
                    $paymentDetails = Payment::create([
                        ...
                        'email_reminder_date'   => $monthly[$i - 1][6]
    ...
    public function resetFilterHomepage(Request $request)
    {
        ...
        if ($trips->count() != 0) {
            foreach ($trips as $trip) {
                $trip['date_from_result'] = date('d M Y', strtotime($trip->date_from));
